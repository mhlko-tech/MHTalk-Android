...
object MembershipService {
    ...
    suspend fun sync(context: Context, accessToken: String): SyncResult? = withContext(Dispatchers.IO) {
        val token = SecureTokenStore(context.applicationContext).get(TOKEN_KEY) ?: return@withContext null
        val origin = BuildConfig.TOKEN_ENDPOINT.substringBefore("/livekit/token")
        val request = Request.Builder()
            .url("$origin/subscription/lava/sync")
            .header("Authorization", "Bearer $accessToken")
        ...
        client.newCall(request).execute().use { response ->
            ...
            SyncResult(
                ...
                tier = if (payload.optString("tier") == "plus") SubscriptionTier.Plus else SubscriptionTier.Free,
                pending = payload.optBoolean("pending"),
            )
        }
    }

    private const val TOKEN_KEY = "mhtalk.membership.lava-token"
}
